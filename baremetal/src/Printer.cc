...
void Printer::doNothing() { ebbrt::kprintf("doNothing\n"); }
...
void Printer::ReceiveMessage(ebbrt::Messenger::NetworkId nid,
  ...
  else if (output[0] == 'D') 
  {
	  ...
	  ebbrt::event_manager->SpawnRemote([theCpu, ncpus, &count, &context]() {
	      ...
	      indexToCPU(i));  // if i don't add indexToCPU, one of the cores never run??
      }
  ...
  else if (output[0] == 'F') 
  {
	  ...
	  // spawn jobs on each core using SpawnRemote
	  ebbrt::event_manager->SpawnRemote([theCpu, ncpus, &count, &context, 
	       &_reconstructed, &_quality_factor, 
	       &_mask, &_max_slices, &_slices, 
					     &_transformations, &_volcoeffs, &_slice_inside_cpu, i]() 
	       {
		  // get my cpu id
		  size_t mycpu = ebbrt::Cpu::GetMine();
		  ...
		  runCoeffInit(_mask, _reconstructed,
			       _quality_factor,
			       _max_slices,
			       _slices,
			       _transformations,
